...
l.opts.onmouseup||l.opts.onmousemove||l.opts.tooltip||l.opts.onmouseover||l.opts.onmouseout){l.opts.evented=!0;if(l.opts.onmousedown)l.canvas.on("mouse:down",function(b){if(l.display.image&&b.target){var d=b.target,e=d.params;if(!e||e&&!1!==e.changeable)"main"===l.dtype&&(l.display.image.clickInRegion=!0,l.display.image.clickInLayer=c),l.opts.onmousedown.call(l.canvas,l.display.image,d.pub,b.e,d)}else if(l.canvas._selection=l.canvas.selection)l.canvas.selection=a.specialKey(b.e)});else l.canvas.on("mouse:down",
function(b){if(l.canvas._selection=l.canvas.selection)l.canvas.selection=a.specialKey(b.e)});if(l.opts.onmouseup)l.canvas.on("mouse:up",function(a){l.display.image&&a.target&&l.opts.onmouseup.call(l.canvas,l.display.image,a.target.pub,a.e,a.target);l.canvas.selection=l.canvas._selection||l.canvas.selection});else l.canvas.on("mouse:up",function(){l.canvas.selection=l.canvas._selection||l.canvas.selection});if(l.opts.onmousemove)l.canvas.on("mouse:move",function(a){l.display.image&&a.target&&l.opts.onmousemove.call(l.canvas,
l.display.image,a.target.pub,a.e,a.target)});if(l.opts.onmouseover)l.canvas.on("mouse:over",function(a){l.display.image&&a.target&&l.opts.onmouseover.call(l.canvas,l.display.image,a.target.pub,a.e,a.target)});if(l.opts.onmouseout)l.canvas.on("mouse:out",function(a){l.display.image&&a.target&&l.opts.onmouseout.call(l.canvas,l.display.image,a.target.pub,a.e,a.target)});if(l.opts.onmousedblclick)l.canvas.on("mouse:dblclick",function(a){l.display.image&&a.target&&l.opts.onmousedblclick.call(l.canvas,
l.display.image,a.target.pub,a.e,a.target)});l.opts.tooltip&&(l.canvas.on("mouse:over",function(b){l.display.image&&b.target&&a.tooltip(b.target.left+b.target.width+2,b.target.top+b.target.height+2,l.opts.tooltip,l.display.image,b.target.pub,b.e,b.target)}),l.canvas.on("mouse:out",function(b){l.display.image&&b.target&&a.tooltip(b.target.left,b.target.top,null,l.display.image,b.target.pub,b.e,b.target)}))}else l.canvas.on("mouse:down",function(b){if(l.canvas._selection=l.canvas.selection)l.canvas.selection=
a.specialKey(b.e)}),l.canvas.on("mouse:up",function(){l.canvas.selection=l.canvas._selection||l.canvas.selection});l.canvas.on("object:modified",function(b){var c=[];if(b.target){var d=b.target;a.Fabric.updateChildren(l,d,"deltas");if(l.opts.sortOverlapping&&(d.setCoords(),l.canvas.forEachObject(function(a){a!==d&&d.intersectsWithObject(a)&&(e=a.getScaledWidth(),f=a.getScaledHeight(),c.push({obj:a,siz:e*f}))}),c.length)){var e=d.getScaledWidth();var f=d.getScaledHeight();c.push({obj:d,siz:e*f});c.sort(function(a,
b){return a.siz<=b.siz?-1:1});var g=c.length;for(b=0;b<g;b++)try{c[b].obj.sendToBack()}catch(w){}}}});l.canvas.on("object:scaling",function(b){b.target&&(b=b.target,b.rescaleEvenly(),b.rescaleBorder(),a.Fabric.updateChildren(l,b,"scaling"),"activeSelection"!==b.type&&"group"!==b.type||f(b,"scaling"))});l.canvas.on("object:moving",function(b){b.target&&(b=b.target,a.Fabric.updateChildren(l,b,"moving"),"activeSelection"!==b.type&&"group"!==b.type||f(b,"moving"))});l.canvas.on("object:rotating",function(b){b.target&&
(b=b.target,a.Fabric.updateChildren(l,b,"rotating"),"activeSelection"!==b.type&&"group"!==b.type||f(b,"rotating"))});l.canvas.on("selection:created",function(b){if(!a.globalOpts.skipSelectionProcessing)if(b.target){var c=b.target;"activeSelection"===c.type||"group"===c.type&&!c.params?m(l,b,c):k(l,c)}else b.selected&&b.selected.length&&(1===b.selected.length?(c=b.selected[0],k(l,c)):m(l,b))});l.canvas.on("selection:updated",function(b){if(!a.globalOpts.skipSelectionProcessing)if(b.target){var c=b.target;
"activeSelection"===c.type||"group"===c.type&&!c.params?m(l,b,c):k(l,c)}else b.selected&&b.selected.length&&(1===b.selected.length?(c=b.selected[0],k(l,c)):m(l,b))});l.canvas.on("before:selection:cleared",function(b){if(!a.globalOpts.skipSelectionProcessing&&b.target)if(b=b.target,"activeSelection"===b.type||"group"===b.type&&!b.params){b=l;var c;var d=b.canvas.getActiveObjects();for(c=0;c<d.length;c++){var e=d[c];h(b,e)}g(-1)}else h(l,b)});if(l.divjq.closest(a.lightOpts[a.LIGHTWIN].drag).length)if(fabric.isTouchSupported)l.divjq.on("touchstart",
...
(m=180*-m/Math.PI,b.angle+=m)}a.notNull(this.params.transformAngle)&&(b.angle+=this.params.transformAngle);a.notNull(this.params.transformScale)&&(b.angle*=this.params.transformScale)}f.angle=-b.angle}void 0!==b.dx&&void 0!==b.dy&&(f.left=b.dx,f.top=b.dy);void 0!==b.x&&void 0!==b.y&&(m=this.imageToDisplayPos(b),f.left=m.x,f.top=m.y);void 0!==b.px&&void 0!==b.py&&(m=this.logicalToDisplayPos({x:b.px,y:b.py}),f.left=m.x,f.top=m.y);"string"===typeof b.wcs&&(m=b.wcs.trim().split(/ +/),b.ra=m[0],b.dec=
m[1],3<=m.length&&(b.wcssys=m[2]));if(this.validWCS()&&a.notNull(b.ra)&&a.notNull(b.dec)){if(b.wcssys){var l=this.getWCSSys();var p=a.globalOpts.xeqPlugins;a.globalOpts.xeqPlugins=!1;this.setWCSSys(b.wcssys,!1)}else b._wcssys&&(l=this.getWCSSys(),p=a.globalOpts.xeqPlugins,a.globalOpts.xeqPlugins=!1,this.setWCSSys(b._wcssys,!1),delete b._wcssys);"string"===typeof b.ra&&(b.ra=a.saostrtod(b.ra),a.isHMS(this.params.wcssys)&&(b.ra*=15));"string"===typeof b.dec&&(b.dec=a.saostrtod(b.dec));m=a.wcs2pix(this.raw.wcs,
b.ra,b.dec).trim().split(/ +/);l&&(this.setWCSSys(l,!1),a.globalOpts.xeqPlugins=p);m=this.imageToDisplayPos({x:parseFloat(m[0]),y:parseFloat(m[1])});f.left=m.x;f.top=m.y}void 0!==b.left&&(f.left=b.left);void 0!==b.top&&(f.top=b.top);void 0===f.left&&!0!==b.noLeftTop&&(f.left=d&&void 0!==d.left?d.left:this.display.canvasjq.attr("width")/2-1);void 0===f.top&&!0!==b.noLeftTop&&(f.top=d&&void 0!==d.top?d.top:this.display.canvasjq.attr("height")/2-1+1);b.deltax&&(f.left+=b.deltax);b.deltay&&(f.top-=b.deltay);
c="main"!==this.display.layers[c].dtype||b.preservedcoords?1:this.rgb.sect.zoom;if(""===b.strokeDashes||""===b.strokeDashArray)b.strokeDashArray=[];""===b.strokeWidth&&(b.strokeWidth=a.Fabric.opts.strokeWidth);switch(b.shape){case "annulus":g.radii=[];if(void 0!==b.radii)if("string"===typeof b.radii)for(g.radii=b.radii.replace(/ /g,"").split(","),p=e=0;e<g.radii.length;e++)""!==g.radii[e]&&(b.sizeScale&&(g.radii[e]*=b.sizeScale),g.radii[p++]=this.wcs2imlen(g.radii[e]));else{if(g.radii=b.radii,b.sizeScale)for(e=
0;e<g.radii.length;e++)g.radii[e]*=b.sizeScale}else for(b.ireg&&a.SCALEIREG&&(a.notNull(b.iradius)&&(b.iradius/=c),a.notNull(b.oradius)&&(b.oradius/=c)),b.sizeScale&&(a.notNull(b.iradius)&&(b.iradius*=b.sizeScale),a.notNull(b.oradius)&&(b.oradius*=b.sizeScale)),c=(b.oradius-b.iradius)/b.nannuli,l=b.nannuli+1,e=0;e<l;e++)p=b.iradius+c*e,b.sizeScale&&(p*=b.sizeScale),g.radii.push(p);break;case "box":case "cross":b.ireg&&a.SCALEIREG&&(a.notNull(b.width)&&(b.width/=c),a.notNull(b.height)&&(b.height/=
